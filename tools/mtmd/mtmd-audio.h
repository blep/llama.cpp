...
struct mtmd_audio_preprocessor_voxtral_realtime : mtmd_audio_preprocessor {
    mtmd_audio_preprocessor_voxtral_realtime(const clip_ctx * ctx) : mtmd_audio_preprocessor(ctx) {}
    void initialize() override;
    bool preprocess(const float * samples, size_t n_samples, std::vector<mtmd_audio_mel> & output) const override;
